...
class FormField:
    def __init__(self, label: str, default: Any = "", required: bool = True, field_type: str = "text",
                 options: List[str] = None, help_text: str = ""):
        self.label = label
        self.default = default
        self.required = required
        self.field_type = field_type
        self.options = options
        self.help_text = help_text  # Добавляем поле для подсказки
        self.value = default
        self.error = ""
...
class Form:
    def __init__(self, title: str, fields: List[FormField], clear_screen: bool = True):
        ...
        self.first_render = True  # Флаг для первого рендера
    ...
    def display(self) -> Optional[Dict[str, Any]]:
        while True:
            if self.clear_screen:
                ...
                # При clear_screen=False рисуем только один раз
                if self.first_render:
                    ConsoleUI.print_header(self.title)
                    self._render_fields()
                    self.first_render = False
                else:
                    # Обновляем только строку с editing
                    if self.editing:
                        ...
                        # Перерисовываем только статусную строку
                        console.print("\n" + " " * 50 + "\r", end="")  # Очищаем последнюю строку
                        console.print("[dim]Enter: edit field | Esc: cancel | Ctrl+S: submit[/dim]", end="")
            ...
            if key == 'up' and not self.editing:
                ...
                if not self.clear_screen:
                    # Обновляем отображение выбранного поля
                ...
                if not self.clear_screen and not self.editing:
                    # Перерисовываем значение поля
    ...
    def _update_selection(self):
        """Обновляет отображение выбранного поля (без перерисовки всего)"""
        # Поднимаемся на нужное количество строк
        # Это сложно реализовать без полной перерисовки, поэтому для простоты
        # делаем полную перерисовку но без очистки экрана
        console.print("\n" * (len(self.fields) + 3), end="")  # Отступаем
        self._render_fields()
    
    def _update_field_value(self):
        """Обновляет значение конкретного поля"""
        # Аналогично, для простоты делаем полную перерисовку
...
Let's set up your environment!
    ...
    def create_new_graph(self) -> bool:
        ...
        if text_path.suffix == '.csv':
            # Показываем колонки ДО формы (без очистки экрана)
            ConsoleUI.clear()  # Очищаем один раз перед показом колонок
            ...
            # Создаем форму БЕЗ очистки экрана
            csv_fields = [
                FormField("Columns to use (comma-separated)", 
                        required=False)
            ]
    ...
    """
        ...
        if self.graph_filepath:
            ...
            if history_path.exists():
                ...
                console.print(f"[dim]✓ Loaded chat history for graph: {graph_name}[/dim]")
            else:
                self.chat_history = ChatHistory(
                    graph_path=self.graph_filepath,
                    file_path=str(history_path)
                )
                console.print(f"[dim]✓ Created new chat history for graph: {graph_name}[/dim]")
        else:
        ...
        while True:
            ...
            if query.lower() in ['/quit', '/q']:
                ...
                current_query = ""
                current_message_history = ""
                console.print("[green]✓ Local session history cleared![/green]")
                continue
            ...
            with Live(status_text, refresh_per_second=10, transient=True) as live:
                try:
                    ...
                    plan = generate_plan(query, final_context, self.chat_model)
                    
                    status_text = Text("✍️ Writing answer...", style="cyan")
                    live.update(status_text)
                    answer = generate_answer_based_on_plan(query, plan, final_context, self.chat_model)
